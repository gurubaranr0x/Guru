...
var _svgPathExp = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi, _numbersExp = /(?:(-)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi, _scientific = /[\+\-]?\d*\.?\d+e[\+\-]?\d+/gi, _selectorExp = /(^[#\.][a-z]|[a-y][a-z])/i, _DEG2RAD = Math.PI / 180, _RAD2DEG = 180 / Math.PI, _sin = Math.sin, _cos = Math.cos, _abs = Math.abs, _sqrt = Math.sqrt, _atan2 = Math.atan2, _largeNum = 1e8, _isString = function _isString$1(value) {
	return typeof value === "string";
}, _isNumber = function _isNumber$1(value) {
	return typeof value === "number";
}, _isUndefined = function _isUndefined$1(value) {
	return typeof value === "undefined";
}, _temp = {}, _temp2 = {}, _roundingNum = 1e5, _wrapProgress = function _wrapProgress$1(progress) {
	return Math.round((progress + _largeNum) % 1 * _roundingNum) / _roundingNum || (progress < 0 ? 0 : 1);
}, _round$1 = function _round$2(value) {
	return Math.round(value * _roundingNum) / _roundingNum || 0;
}, _roundPrecise = function _roundPrecise$1(value) {
	return Math.round(value * 1e10) / 1e10 || 0;
}, _segmentIsClosed = function _segmentIsClosed$1(segment) {
	return segment.closed = Math.abs(segment[0] - segment[segment.length - 2]) < .001 && Math.abs(segment[1] - segment[segment.length - 1]) < .001;
}, _splitSegment = function _splitSegment$1(rawPath, segIndex, i, t) {
...
function stringToRawPath(d) {
	...
	}).match(_svgPathExp) || [], path = [], relativeX = 0, relativeY = 0, twoThirds = 2 / 3, elements = a.length, points = 0, errorMessage = "ERROR: malformed path: " + d, i, j, x, y, command, isRelative, segment, startX, startY, difX, difY, beziers, prevCommand, flag1, flag2, line = function line$1(sx, sy, ex, ey) {
		difX = (ex - sx) / 3;
		difY = (ey - sy) / 3;
...
function rawPathToString(rawPath) {
	...
	for (s = 0; s < l; s++) {
		...
		sl = segment.length;
		for (i = 2; i < sl; i++) result += _round$1(segment[i++]) + "," + _round$1(segment[i++]) + " " + _round$1(segment[i++]) + "," + _round$1(segment[i++]) + " " + _round$1(segment[i++]) + "," + _round$1(segment[i]) + " ";
...
var gsap, _coreInitted, _getGSAP = function _getGSAP$1() {
...
}, _bigNum = 0x56bc75e2d63100000, _round = function _round$2(value) {
	return ~~(value * 1e3 + (value < 0 ? -.5 : .5)) / 1e3;
}, _bonusValidated = 1, _numExp = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/gi, _needsParsingExp = /[cLlsSaAhHvVtTqQ]/g, _findMinimum = function _findMinimum$1(values) {
...
var CustomEase = /* @__PURE__ */ function() {
	...
	_proto.setData = function setData(data, config) {
		...
		this.ease = function(p$1) {
			var point$1 = lookup[p$1 * l | 0] || lookup[l - 1];
			if (point$1.nx < p$1) point$1 = point$1.n;
			return point$1.y + (p$1 - point$1.x) / point$1.cx * point$1.cy;
		};
	...
	CustomEase$1.getSVGData = function getSVGData(ease, config) {
		...
		if (ease instanceof CustomEase$1) a = rawPathToString(transformRawPath([ease.segment.slice(0)], width, 0, 0, -height, x, y));
		else {
			a = [x, y];
